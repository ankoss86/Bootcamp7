...
const Card = ({getFullInfo, getFullHandler, addToFavoritesHendler, id, img}) => {
    return (
        ...
        <div className="overlay">
            ...
            <div className="overlayIcon"><span data-title='full info' className="tooltip"></span><NavLink to={`/more/${id}`}><i id={id} className="hidenIcons fas fa-expand-arrows-alt fa-3x"></i></NavLink></div>
        </div>
        </div>
    );
};
...
function MSTP (state) {
    return {
        results: state.getNowPaying,
    }
}

function MDTP (dispatch) {
    return {
        getFullInfo: function(event){
            dispatch(asyncFullInfo(event))
        }
    }
}
