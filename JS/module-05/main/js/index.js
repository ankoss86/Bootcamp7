...
const getUsersByEyeColor = function(arr, color){
    return arr.filter(el =>el.eyeColor.includes(color));
};
...
const getTotalBalance = function(arr){
    return (arr.map(el => el.balance)).reduce((acc, el) => acc+el, 0);
};
...
const getUsersByFriend = function(arr, name){
    return (arr.filter(el => (el.friends).includes(name))).map(el => el.name);  
};
...
//Получить массив всех скиллов всех пользователей (поле skills), 
...
const getUserNamesSortedByFriendsCount = function(arr){
    return (arr.sort((a,b) => (a.friends).length > (b.friends).length)).map(el => el.name);
};
